...
typedef union {
    ...
    struct{
        ...
        bool	Reserved_1					: 1; // 14
        bool	Reserved_2					: 1; // 15
    }Bit;
}Detector_Status_1;
...
static void MODBUS_Map_Remote_Update_Check( void );
/* End Function */
...
extern  char *LWIP_TFTP_EXAMPLE_CLIENT_FILENAME;

s_FWupdate_flag tftp_flag;
extern u_Nand_Page nand;
extern s_FWupdate FWupdate; 
/* End Variable */
...
void MODBUS_Mapping( void )
{
	...
	Dsc_Input[0].Bit.Function = pFunc->State.Bit.IsEntered;
	...
	Dsc_Input[0].Bit.Maintenance = MeasHandle.Mode.Item.Maintenance;
	...
	if( pMeas->Conf.OutMax > 1 ){
		...
		Dsc_Input[1].Bit.Function = pFunc->State.Bit.IsEntered;
		...
		Dsc_Input[1].Bit.Maintenance = MeasHandle.Mode.Item.Maintenance;
	...
	Det_Status_1[0].Bit.Function = Dsc_Input[0].Bit.Function;
	Det_Status_1[1].Bit.Function = Dsc_Input[1].Bit.Function;
...
void MODBUS_Map_Update_Check( void )
{
	...
	S16			SetMax;
        ...
        MODBUS_Map_Remote_Update_Check();
...
static void MODBUS_Map_Remote_Update_Check( void )
{     
      if (Modbus_Reg_4xxxx[MODBUS_40090] == 0x0001) {
          tftp_flag.bMain_Update = Modbus_Reg_0xxxx[1].Bit._2;
          tftp_flag.bCt_Update   = Modbus_Reg_0xxxx[1].Bit._1; 
          tftp_flag.bIr_Update   = Modbus_Reg_0xxxx[1].Bit._0;
          
          nand.byte[0] = tftp_flag.bMain_Update;
          nand.byte[1] = tftp_flag.bCt_Update;
          nand.byte[2] = tftp_flag.bIr_Update;
          
          printf(" Erase 14Block \r\n");
          NAND_Erase_Block(0, 14, 0);
          NAND_Write_Page_8bit(0, 14, 0, nand.byte);
          
          FWupdate.seq = START_UPDATE;     
      } 
      //else if (Modbus_Reg_4xxxx[MODBUS_40090] == 0x0002) {

      //    NAND_Read_Page_8bit(0, 14, 0, nand.byte);
      //    printf(" %x,%x,%x \n", nand.byte[0], nand.byte[1], nand.byte[2]);   
      //}
}
			...
			switch( pInf->SenNo ){
				...
				case IR_SEN_03_370:
					switch( pInf->GasNo ){
						...
						case IR_GAS_03_370_BTBAS_1000_PPM:			No = 115; break;
					}
					break;
				...
				case IR_SEN_08_000:
					switch( pInf->GasNo ){
						...
						case IR_GAS_08_000_CF4_500_PPM:				No = 114; break;
					}
					break;
				...
				case IR_SEN_10_990:
					switch( pInf->GasNo ){
						...
						case IR_GAS_10_990_C2H4_100_LEL:			No =   8; break;
					}
					break;
			}
			break;
