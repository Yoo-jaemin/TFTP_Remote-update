...
{   
    ...
    { 
      if (NAND_Erase_Block(0, i, 0) == false)           
        printf("\r\n Erase Block Err\r\n");
    ...
    NandAddr.Block = 15;
    NandAddr.Plane = 0;                  
    nand.byte[0] = mainFlag; // update check byte
    HAL_NAND_Write_Page_8b(&NandBank3_Handle, &NandAddr, nand.byte, 1);

    NandAddr.Page  = 1;
    NandAddr.Block = 16;
    NandAddr.Plane = 0;                  
    nand.byte[0] = CtFlag; // update check byte
    HAL_NAND_Write_Page_8b(&NandBank3_Handle, &NandAddr, nand.byte, 1);

    NandAddr.Page  = 1;
    NandAddr.Block = 17;
    NandAddr.Plane = 0;                  
    nand.byte[0] = IrFlag; // update check byte
    HAL_NAND_Write_Page_8b(&NandBank3_Handle, &NandAddr, nand.byte, 1);
}
