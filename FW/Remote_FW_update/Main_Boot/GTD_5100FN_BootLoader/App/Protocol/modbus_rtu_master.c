...
void MODBUS_RTU_Write_Register_Packet_Send_Master( MODBUS_MasterHandleType *pMaster )
{
	...
	switch( pMaster->Function ){
		case WRITE_SINGLE_REGISTER:  
			...
			USART3_Packet_Send(pMaster->TxPacket, pMaster->TxLength);
                        break;
...
void Cartridge_BootJump_Write_Packet_Send( MODBUS_MasterHandleType *pModbus ) 
{
    printf("\r\n Cartridge_BootJump_CMD_Packet_Send \r\n");
    
    pModbus->SlaveAddress       = 0;    //CARTRIDGE_MODBUS_ADDRESS
    pModbus->Function           = WRITE_SINGLE_REGISTER;
    pModbus->StartAddress       = 0x0000;
    pModbus->pRequestBuf        = &g_value;
...
void Cavity_BootJump_Write_Packet_Send( MODBUS_MasterHandleType *pModbus )
{
    printf("\r\n Cavity_BootJump_CMD_Packet_Send \r\n");
    
    pModbus->SlaveAddress       = 1; //CAVITY_MODBUS_ADDRESS
    pModbus->Function           = WRITE_SINGLE_REGISTER;
    pModbus->StartAddress       = 0x0000;
    pModbus->pRequestBuf        = &g_value;
    pModbus->Status	        = MODBUS_RTU_MASTER_BUSY;
