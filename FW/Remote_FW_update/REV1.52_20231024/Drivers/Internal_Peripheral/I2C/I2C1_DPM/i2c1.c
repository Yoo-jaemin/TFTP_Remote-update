...
bool I2C1_Write( U16 DevAddress, U16 RegAddress, U8 *pData, U16 Size )
{
	U8		i;
	bool	err = false;
	...
	for( i=0; i<RETRY_COUNT; i++ ){
		...
		if( HAL_I2C_Mem_Write( &I2C1_Handle, DevAddress, RegAddress, I2C_MEMADD_SIZE_8BIT, pData, Size, TIMEOUT ) == HAL_OK ){
			break;
		}
		else {
...
bool I2C1_Read( U16 DevAddress, U16 RegAddress, U8 *pData, U16 Size )
{
	U8		i;
	bool	err = false;
	...
	for( i=0; i<RETRY_COUNT; i++ ){
		...
		if( HAL_I2C_Mem_Read( &I2C1_Handle, DevAddress, RegAddress, I2C_MEMADD_SIZE_8BIT, pData, Size, TIMEOUT ) == HAL_OK ){
			break;
		}
		else {
			if( HAL_I2C_GetError( &I2C1_Handle ) != HAL_I2C_ERROR_AF ){
