...
bool I2C2_Write( uint16_t DevAddr, uint16_t MemAddr, uint8_t *pData, uint16_t Size )
{
	U8		i;
	bool	err = false;
	...
	for( i=0; i<RETRY_COUNT; i++ ){
		...
		if( HAL_I2C_Mem_Write( &I2C2_Handle, DevAddr, MemAddr, I2C_MEMADD_SIZE_16BIT, pData, Size, TIMEOUT ) == HAL_OK ){
			break;
		}
		else {
...
bool I2C2_Read( uint16_t DevAddr, uint16_t MemAddr, uint8_t *pData, uint16_t Size )
{
	U8		i;
	bool	err = false;
	
	for( i=0; i<RETRY_COUNT; i++ ){
		if( HAL_I2C_Mem_Read( &I2C2_Handle, DevAddr, MemAddr, I2C_MEMADD_SIZE_16BIT, pData, Size, TIMEOUT ) == HAL_OK ){
			break;
		}
		else {
			if( HAL_I2C_GetError( &I2C2_Handle ) != HAL_I2C_ERROR_AF ){
